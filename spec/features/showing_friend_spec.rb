...
RSpec.feature 'Showing Friend Workout' do
  before do
    ...
    @e1 = @john.exercises.create(duration_in_min: 74, workout_details: 'Weight lifting routine',
                                 workout_date: Date.today)
    @e2 = @sarah.exercises.create(duration_in_min: 55, workout_details: 'Sarahs weight lifting routine',
                                  workout_date: Date.today)
    login_as(@john)
  ...
  scenario 'shows friend\'s workout for last 7 days' do
    ...
    expect(page).to have_content(@e2.workout_details)
